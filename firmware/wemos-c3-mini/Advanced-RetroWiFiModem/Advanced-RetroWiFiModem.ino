//
//   Retro WiFi modem: ESP32-C3 (LOLIN C3 Mini) RS232<->WiFi Hayes modem
...
void setup(void) {
   ...
   // TCP_NODELAY is set per-client on connect (see dialNumber/answerCall)
   ...
   if( WiFi.status() == WL_CONNECTED || !settings.ssid[0] ) {
      ...
      sendResult(RC_ERROR);           // SSID configured, but not connected
   }
  ...
  Serial.println("   2021 - 2025 | LOLIN C3 Mini Hayes Modem"                   );
  Serial.println("   Hayes AT | PPP+NAT | DFU experimental"                    );
  Serial.println("============================================================");
  Serial.println();
...
void loop(void) {
   ...
   switch( state ) {
      ...
      case ONLINE_PPP:
         // Before PPP link is up, serial traffic is still Hayes commands (pppd
         // chat script). After link-up only PPP frames are valid (HDLC 0x7E…).
         if( !pppWasLinkUp() && Serial.available() ) {
            int c = Serial.peek();
            if( c == 'A' || c == 'a' ) {
               inAtCommandMode();
               break;
         ...
         if( pppLinkDropped() && state == ONLINE_PPP ) {
            endCall();
         }
         break;
   }
...
void doAtCmds(char *atCmd) {
   ...
   if( atCmd[0] ) {
      ...
      if( strncasecmp(atCmd, "AT", 2) ) {
         sendResult(RC_ERROR); // nope, time to die
      } else {
         ...
         if( !atCmd[0] ) {
            ...
            while( atCmd[0] ) {
               ...
               } else if( !strncasecmp(atCmd, "D", 1) && atCmd[1] == '*' ) {
                  atCmd = dialPpp(atCmd + 1);
               ...
               } else if( !strncasecmp(atCmd, "$DFU", 4) ) {
                  atCmd = doDfu(atCmd + 4);
