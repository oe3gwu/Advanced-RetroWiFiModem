   ...
   enum ResultCodes { RC_OK, RC_CONNECT, RC_RING, RC_NO_CARRIER, RC_ERROR, RC_NO_ANSWER, RC_RING_IP };
   const char * const resultCodes[] PROGMEM = { okStr, connectStr, ringStr, noCarrierStr, errorStr, noAnswerStr, ringStr};
   #define R_OK RC_OK
   #define R_CONNECT RC_CONNECT
   #define R_RING RC_RING
   #define R_NO_CARRIER RC_NO_CARRIER
   #define R_ERROR RC_ERROR
   ...
   enum {CMD_NOT_IN_CALL, CMD_IN_CALL, ONLINE, PASSWORD, ONLINE_PPP} state = CMD_NOT_IN_CALL;
