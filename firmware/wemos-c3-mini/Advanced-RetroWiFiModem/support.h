...
void sendResult(int resultCode) {
   ...
   } else if( resultCode == RC_ERROR ) {
      lastCmd[0] = NUL;
      memset(atCmd, 0, sizeof atCmd);
   }
   if( resultCode == RC_NO_CARRIER || resultCode == RC_NO_ANSWER ) {
   ...
   switch( settings.dataBits ) {
      case 5:
         switch( settings.parity ) {
            case 'E':
               return settings.stopBits == 2 ? SERIAL_5E2 : SERIAL_5E1;
            case 'O':
               return settings.stopBits == 2 ? SERIAL_5O2 : SERIAL_5O1;
            default:
               return settings.stopBits == 2 ? SERIAL_5N2 : SERIAL_5N1;
         }
      case 6:
         switch( settings.parity ) {
            case 'E':
               return settings.stopBits == 2 ? SERIAL_6E2 : SERIAL_6E1;
            case 'O':
               return settings.stopBits == 2 ? SERIAL_6O2 : SERIAL_6O1;
            default:
               return settings.stopBits == 2 ? SERIAL_6N2 : SERIAL_6N1;
         }
      case 7:
         switch( settings.parity ) {
            case 'E':
               return settings.stopBits == 2 ? SERIAL_7E2 : SERIAL_7E1;
            case 'O':
               return settings.stopBits == 2 ? SERIAL_7O2 : SERIAL_7O1;
            default:
               return settings.stopBits == 2 ? SERIAL_7N2 : SERIAL_7N1;
         }
      case 8:
      default:
         switch( settings.parity ) {
            case 'E':
               return settings.stopBits == 2 ? SERIAL_8E2 : SERIAL_8E1;
            case 'O':
               return settings.stopBits == 2 ? SERIAL_8O2 : SERIAL_8O1;
            default:
               return settings.stopBits == 2 ? SERIAL_8N2 : SERIAL_8N1;
         }
   }
}

// LOLIN C3 Mini as modem (DCE): RTS/CTS naming matches the RS232 shield.
void setHardwareFlow(void) {
   // CTS (GPIO5 / D8): output, tells DTE to pause sending when RX buffer fills
   // RTS (GPIO4 / D7): input, pauses ESP32 TX when DTE asserts it
   Serial.setPins(-1, -1, CTS, RTS);
...
void displayCurrentSettings(void) {
   ...
   Serial.printf("E%u Q%u V%u X%u &D%u &K%u NET%u S0=%u S2=%u\r\n",
...
void displayStoredSettings(void) {
   ...
   Serial.printf("E%u Q%u V%u X%u &D%u &K%u NET%u S0=%u S2=%u\r\n",
      ...
      EEPROM.get(offsetof(struct Settings, dtrHandling), v_uint8),
